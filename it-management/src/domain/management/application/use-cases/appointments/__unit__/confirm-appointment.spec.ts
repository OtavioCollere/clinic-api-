...
describe("Confirm appointment use case unit tests", () => {
  ...
  it("should be able to confirm an appointment", async () => {
    ...
    if (isRight(result)) {
      expect(unwrapEither(result).appointment.status).toBe('CONFIRMED')
    }
  });
  ...
  it("should not be able to confirm an appointment with a non-existent user", async () => {
    const appointment = MakeAppointment({});
    inMemoryAppointmentsRepository.items.push(appointment);
