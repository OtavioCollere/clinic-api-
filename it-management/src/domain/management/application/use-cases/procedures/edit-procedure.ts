...
export class EditProcedureUseCase{
  ...
  async execute({procedureId, userId, appointmentId, name, product, region, value} : EditProcedureUseCaseRequest) : Promise<EditProcedureUseCaseResponse> {
    ...
    if (product) procedure.product = product
    if (region) procedure.region = region
